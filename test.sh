...
config_sites="$repo_dir/config/sites.test.yml"
examples_sites="$repo_dir/examples/sites.test.yml"
...
sed \
	...
	"$repo_dir/config/sites.yml" > "$config_sites"
...
sed \
	...
	-e "s|/root/rpsiod/examples/www/errors|$repo_dir/examples/www/errors|g" \
	-e "s|/absolute/path/to/rpsiod/examples/www/errors|$repo_dir/examples/www/errors|g" \
	"$repo_dir/examples/sites.yml" > "$examples_sites"

python3 "$repo_dir/tests/security_regression.py"
python3 "$repo_dir/tests/streaming_regression.py"

"$repo_dir/build/rpsiod" test --server "$repo_dir/config/server.yml" --sites "$config_sites"
"$repo_dir/build/rpsiod" configtest --server "$repo_dir/examples/server.yml" --sites "$examples_sites"
...
if [[ "$php_body" == "PHP_OK" ]]; then
	if curl -s http://127.0.0.1/php-test.php | grep -q '<?php'; then
		...
		rm -f "$config_sites" "$examples_sites"
		exit 1
	fi
